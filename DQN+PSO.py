...
# SEARCHER---------------------------------------------------------------------------------------------
lb, ub, dim = -1, 1, in_dim * hid1_dim + hid1_dim * hid2_dim + hid2_dim * out_dim
# PHASE = 0.1
...
class Brain:
    def __init__(self, num_states, num_actions, epsilon, swarm_no, wolves_no, w, c1, c2, lb, ub):
        ...
        self.population = np.zeros((self.swarm_no, self.dim))
        self.velocity = np.zeros((self.swarm_no, self.dim))
        self.particle_best_pos = np.zeros((self.swarm_no, self.dim))
        self.particle_best_sco = np.zeros((self.swarm_no,))
        self.swarm_best_pos = np.zeros(self.dim)
        self.swarm_best_sco = float("inf")
    ...
    def replay(self):
        ...
        if len(self.memory) > BATCH_SIZE and self.toggle == False:
            ...
            for i in range(swarm_no):
                ...
                self.velocity[i, :] = np.random.uniform(self.lb, self.ub, self.dim)
                w1, w2, w3 = self.weight_array2tensor(self.population[i, :])
                init_fit = self.get_fitness(w1, w2, w3)

                self.particle_best_pos[i, :] = self.population[i, :]
                self.particle_best_sco[i] = init_fit

                if self.swarm_best_sco > init_fit:
                    self.swarm_best_pos = self.population[i, :]
                    self.swarm_best_sco = init_fit
    ...
    def search(self):
        ...
        for i in range(swarm_no):
            ...
            velo_cog = self.c1 * r1 * (self.particle_best_pos[i, :].copy() - self.population[i, :].copy())
            velo_soc = self.c2 * r2 * (self.swarm_best_pos.copy() - self.population[i, :].copy())
            self.velocity[i, :] = self.inertia_weight * self.velocity[i, :].copy() + velo_cog.copy() + velo_soc.copy()
            self.population[i, :] = self.population[i, :].copy() + self.velocity[i, :].copy()
            result_pos = self.population[i, :].copy()
            w1, w2, w3 = self.weight_array2tensor(self.population[i, :].copy())
            result_fit = self.get_fitness(w1, w2, w3)

            # update particle_best
            if self.particle_best_sco[i] > result_fit:
                self.particle_best_pos[i] = result_pos
                self.particle_best_sco[i] = result_fit

            # update swarm_best
            if self.swarm_best_sco > result_fit:
                self.swarm_best_pos = result_pos
                self.swarm_best_sco = result_fit

        nw1, nw2, nw3 = self.weight_array2tensor(self.swarm_best_pos)
    ...
    def run(self):
        ...
        for episode in range(MAX_ITERATION):
            ...
            for step in range(MAX_STEPS):
                ...
                # 에피소드 종료 처리
                if done:
                    print('%d Episode:\n Success steps = %d \n최근 10 에피소드의 평균 단계 수 = %.1lf' % (
                    episode, step + 1, episode_suc_list.mean()))
                    print("epsilon value = " + str(self.agent.brain.epsilon))
                    try:
                        print("Last step's loss : "+str(self.agent.brain.swarm_best_sco.item()) + "\n")
                    except:
                        print("...warmup..."+'\n')
                    if (episode % 5 == 0):
                    ...
                    try:
                        self.loss_list.append(self.agent.brain.swarm_best_sco.item())
                    except:
