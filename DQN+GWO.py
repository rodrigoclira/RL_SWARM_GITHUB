...
class Brain:
    def __init__(self, num_states, num_actions, epsilon, wolves_no):
    ...
    def get_fitness(self, w1, w2, w3):
        ...
        a_m[non_final_mask] = self.target_q_network(self.non_final_next_states).detach().max(1)[1]
    ...
    def search(self, episode):
        ...
        for i in range(wolves_no):
            ...
            if fitness * 0.75 < self.alpha_score:
                self.delta_score = self.beta_score  # Update delta
                self.delta_pos = self.beta_pos.copy()
                self.beta_score = self.alpha_score  # Update beta
                self.beta_pos = self.alpha_pos.copy()
            ...
            if fitness * 0.75  > self.alpha_score and fitness * 0.75 < self.beta_score:
                self.delta_score = self.beta_score  # Update delte
                self.delta_pos = self.beta_pos.copy()
                self.beta_score = fitness  # Update beta
                self.beta_pos = self.population[i, :].copy()

            if fitness * 0.75 > self.alpha_score and fitness * 0.75 > self.beta_score and fitness * 0.75 < self.delta_score:
                self.delta_score = fitness  # Update delta
                self.delta_pos = self.population[i, :].copy()
...
class Agent:
    def __init__(self, num_states, num_actions, EPSILON, wolves_no):
        self.brain = Brain(num_states, num_actions, EPSILON, wolves_no)
...
class Environment:
    def __init__(self):
        ...
        self.agent = Agent(num_states, num_actions, EPSILON, wolves_no)
        self.success_list = []
        self.loss_list = []
