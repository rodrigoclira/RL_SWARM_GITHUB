...
class Brain:
    def __init__(self, num_states, num_actions, epsilon, wolves_no, lb, ub):
        ...
        self.memory_toggle = False # 메모리가 샘플링이 가능해질 정도로 충분해지면 토글이 해제되고 본격적인 서칭이 시작됨
    ...
    def replay(self, episode):
        ...
        if len(self.memory) > BATCH_SIZE and self.memory_toggle == False:
            print("----------------------------Enough memory-------------------------------------")
